...
namespace QuakeLR
{
    ...
    public class QuakeCharacterController : MonoBehaviour
    {
        ...
        [SerializeField] 
        [Tooltip("Reads how much seconds player will remember jump (seconds)")]
        private float JumpBuffer = 0.15f;

        [SerializeField] 
        [Tooltip("Max amount of gained velocity for BHop")]
        private float m_MaxVel = 25.0f;
        ...
        private float m_JumpBuffer = -1.0f;
         ...
         * Also remember jump for buffering
        ...
        public void ControllerThink(float deltaTime)
        {
            ...
            if (m_JumpBuffer > 0.0f && m_OnGround){
                Jump();
                m_JumpBuffer = -1.0f;
            }
        ...
        private void EvaluateOnGround()
        {
            ...
            Vector3 origin = m_CharacterController.center + transform.position;
            Vector3 sphereCheckPosition = origin - transform.up * (bodyHalfHeight - bodyRadius + m_CharacterController.skinWidth);
        ...
        {
            float wishSpeed = Mathf.Min(m_WishMoveDirection.magnitude, 1.07f);
            float alignment = Vector3.Dot(m_Velocity, m_WishMoveDirection.normalized);
            ...
            float newSpeed = (m_Velocity + m_WishMoveDirection * accelSpeed).magnitude;
            if (newSpeed > maxAirSpeed)
                accelSpeed = maxAirSpeed - currentSpeed;
